...
export class Reset extends Component {
    ...
    componentWillMount() {
        axios.post("http://18.220.186.45:8090/getuser", { token: (window.location.search).slice(1) })
            .then((resp) => {
                if (!resp.data.result) {
                    this.setState({
    ...
    onreset = () => {
        ...
        if (pass=="" && retry==""){
            ...
            if (pass!==retry){
                ...
                axios.post("http://18.220.186.45:8090/resetpass", { token: (window.location.search).slice(1),password:pass})
                .then((resp) => {
                    if (resp.data.result){
                        this.setState({
                            alert:true
